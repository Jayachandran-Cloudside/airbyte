...
def apply_release_candidates(
    ...
    registry_name: str,
    release_candidate_metadata_entries: Optional[List[LatestMetadataEntry]] = None,
) -> dict:
    if not release_candidate_metadata_entries:
        return registry_entry
    registry_entry = copy.deepcopy(registry_entry)
    registry_entry.setdefault("releases", {})
    registry_entry["releases"]["releaseCandidates"] = {
        entry.metadata_definition.data.dockerImageTag: {
            "rolloutConfiguration": entry.metadata_definition.data.releases.rolloutConfiguration.dict(),
            "registryEntry": generate_registry_entry(entry, spec_cache, registry_name).dict(),
        }
        for entry in release_candidate_metadata_entries
    }
    return registry_entry
...
def generate_registry_entry(
    ...
    registry_entry_with_release_candidates = apply_release_candidates(
        registry_entry_with_spec, spec_cache, registry_name, release_candidate_metadata_entries
    )

    _, ConnectorModel = get_connector_type_from_registry_entry(registry_entry_with_release_candidates)

    return ConnectorModel.parse_obj(registry_entry_with_release_candidates)
...
def generate_and_persist_registry_entry(
    ...
    """
    registry_model = generate_registry_entry(metadata_entry, spec_cache, registry_name, release_candidate_metadata_entries)
...
@asset(
    required_resource_keys={"slack", "root_metadata_directory_manager", "release_candidate_metadata_file_blobs"},
    group_name=GROUP_NAME,
    partitions_def=metadata_partitions_def,
    auto_materialize_policy=AutoMaterializePolicy.eager(max_materializations_per_minute=MAX_METADATA_PARTITION_RUN_REQUEST),
)
...
def registry_entry(
    ...
    matching_release_candidates = find_release_candidates_for_metadata_entry(metadata_entry, release_candidate_metadata_entries)
    release_candidates_metadata = {
        "release_candidates": [rc_entry.file_path for rc_entry in matching_release_candidates] if matching_release_candidates else [],
    }
    if matching_release_candidates:
        PublishConnectorLifecycle.log(
            context,
            PublishConnectorLifecycleStage.REGISTRY_ENTRY_GENERATION,
            StageStatus.IN_PROGRESS,
            f"Found release candidate metadata for {metadata_entry.file_path}",
            user_identifier=user_identifier,
            commit_sha=commit_sha,
        )
    ...
    persisted_registry_entries = {
        registry_name: generate_and_persist_registry_entry(
            ...
            registry_name,
            release_candidate_metadata_entries=matching_release_candidates,
        )
